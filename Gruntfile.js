module.exports = function (grunt) {
  ...
  let config = grunt.file.readJSON( 'config/' + (grunt.option('env') || 'default') + '.json' );
      ...
      html: {files: [{dest: '<%= config.dirs.output %>', src: '*.html', cwd: 'html', expand: true}],
        options: {
          ...
          data: function () {
            ...
            let iconstring = grunt.file.read(config.dirs.generated + 'icons.include');
            iconstring = iconstring.replace(new RegExp('viewBox'), 'style="visibility:hidden;width:0;height:0;" viewBox');

            let layoutstring = grunt.file.read(config.dirs.generated + 'layouts.include');
            ...
            let favicons = grunt.file.read(config.dirs.generated + 'favicon.html');
  ...
  grunt.registerTask('full-deploy', 'Deploys the entire app', function () { let list = [
    [
        // Build node-modules
  ...
  grunt.registerTask('update-css', 'updates CSS', function () { let list = [
  ...
  grunt.registerTask('update-mainjs', 'Updates the main JS file', function () { let list = [
